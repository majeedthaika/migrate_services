...
export function MappingEditor() {
  ...
  const handleSaveMappings = async () => {
    ...
    setSaveError(null);
    try {
      const mappingsToSave = entityMappings.map((mapping, index) => ({
        name: `${mapping.source_service}_${mapping.source_entity}_to_${mapping.target_service}_${mapping.target_entity}_${index}`,
      ...
      const result = await mappingAPI.saveAll(mappingsToSave);
      if (result.error) {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={saving || !mappingsModified}
              variant={mappingsModified ? 'primary' : 'outline'}
              size="sm"
            >
              {saving ? (
